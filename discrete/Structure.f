        ...
        do nr = 1, str % nres - 3
         !   WRITE (*,*) "HOLA +1",str % res(nr) % molres,str % res(nr + 1) % molres
            if (str % res(nr) % molres == str % res(nr + 1) % molres) then
                ...
                ! el crossb )
                str % bonds(str % res(nr) % ica, str % res(nr + 1) % ica) = COVB
                str % bonds(str % res(nr) % ica, str % res(nr + 2) % ica) = COVB
                str % bonds(str % res(nr) % ica, str % res(nr + 3) % ica) = COVB
            endif
        enddo
  ...
  !===============================================================================
   ...
   !    Write(*,*)"NPAIRS ",  str % cov % npairs, count(str % bonds .eq. COVB)
    ...
    subroutine setStepPotCOV(str)
        ...
        str % iData(j, k) = intDataTop(1, COVB, sqrt(str % distat2(j, k))*0.999, 0.)
