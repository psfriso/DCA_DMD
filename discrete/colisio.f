...
subroutine springForce(rpl)
  ...
  !
    ...
    !write(*,*) "force"
    do i = 1, rpl%natom
        do k = 1, rpl%Flist(i) % nats
           
            j = rpl%Flist(i) % iData(k) % patnum
          !  write(*,*) "force" , i ,j ,k
           
            csang = calcSignCosAng(rpl%r, rpl%v, i, j, rpl%natom)
            rij = calcDistDP(rpl%r(i), rpl%r(j))
            rbmin = rpl%Flist(i) % idata(k) % stepPt % step(1) % r
            rbmax = rpl%Flist(i) % idata(k) % stepPt % step(2) % r
            if ((rij .gt. rbmax.and.csang .gt. 0.).or.(rij .lt. rbmin.and.csang .lt. 0.)) then
                call chgmom(i, j, xm, xsum, rpl%r, rpl%v, rpl%natom)
...
subroutine colisioBond( rpl )
   ...
   ! 
    ...
    integer i, j, k
    do i = 1, rpl%natom
        do k = 1, rpl%blist(i) % nats
            j = rpl%blist(i) % iData(k) % patnum
            csang = calcSignCosAng(rpl%r, rpl%v, i, j, rpl%natom)
            rij = calcDistDP(rpl%r(i), rpl%r(j))
            rbmin = rpl%blist(i) % idata(k) % stepPt % step(1) % r
            rbmax = rpl%blist(i) % idata(k) % stepPt % step(2) % r
            if ((rij .gt. rbmax.and.csang .gt. 0.).or.(rij .lt. rbmin.and.csang .lt. 0.)) then
                call chgmom(i, j, xm,xsum, rpl%r, rpl%v, rpl%natom)
             !   write(*,*) "bonded", i, j, blist(i) % idata(k) % xsum, natom
            endif
...
! Overlaps based on contact list
    do i = 1, rpl%natom-1
        do k = 1, rpl%clist(i) % nats
            j = rpl%clist(i) % vecinos(k) % patnum
            !
            if (j .gt. i ) then
                csang = calcSignCosAng(rpl%r, rpl%v, i, j, rpl%natom)
                rij = calcDist2DP(rpl%r(i), rpl%r(j))
                if (rij .lt. rpl%clist(i)%vecinos(k)% dmin.and.csang .lt. 0.) then
                    call chgmom(i, j, xm, xsum , rpl%r, rpl%v, rpl%natom)
                    rpl%ierr = rpl%ierr + 1
                end if
            end if
 ...
 !   write(*,*) " ERRORES ", ierr
    ...
    do i = 1, rpl%natom
        do k = 1, rpl%nblist(i) % nats
            j = rpl%nblist(i) % iData(k) % patnum
            l = rpl%nblist(i) % iData(k) % simp
            if (j .gt. i) then
                ! AQUI CAMBIO
                    call updateTCol(i,rpl% temps, rpl%r, rpl%v, rpl%nblist(i) % iData(k), TMIN, rpl%natom)
                    rpl%nblist(j) % iData(l) % timp = rpl%nblist(i) % iData(k) % timp
                    rpl%nblist(j) % iData(l) % deltak = rpl%nblist(i) % iData(k) % deltak
            endif
        enddo
    enddo
...
!===============================================================================!
!===============================================================================
!!subroutine colisioNonBond2(stepPts, temps, nblist, r, v, xm, ierr, &
!    TMIN, natom, isolv)
!    use geometry
!    use geometryDP
!    use intList
...
!        enddo
!    enddo
!end subroutine colisioNonBond2
!!===============================================================================
...
subroutine nextCol(mem1, mem2, np1, np2, tevent, ipart, tpart, natom, nblist)
    ...
    integer, volatile,intent(OUT) :: mem1, mem2
